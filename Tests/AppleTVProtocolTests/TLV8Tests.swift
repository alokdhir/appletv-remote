...
final class TLV8Tests: XCTestCase {
    ...
    func testDecodeTruncatedHeaderThrows() {
        // Only tag byte, no length — must throw truncatedHeader.
        let data = Data([0x06])
        XCTAssertThrowsError(try TLV8.decode(data)) { error in
            XCTAssertEqual(error as? TLV8.DecodeError, .truncatedHeader)
        }
    }

    func testDecodeUndersizedValueThrows() {
        // Tag 0x03, declared len=255, but only 10 bytes follow.
        var data = Data([0x03, 0xFF])
        data.append(contentsOf: Data(repeating: 0xAA, count: 10))
        XCTAssertThrowsError(try TLV8.decode(data)) { error in
            XCTAssertEqual(error as? TLV8.DecodeError,
                           .undersizedValue(declared: 255, available: 10))
        }
    }

    func testDecodeReassemblesFragmentedValue() throws {
    ...
    func testDecodeReassemblesThreeFragments() throws {
    ...
    func testDecodeDoesNotMergeNonConsecutiveFragments() throws {
    ...
    func testRoundTripSingleEntry() throws {
        ...
        let decoded = try TLV8.decode(tlv.encode())
        XCTAssertEqual(decoded[.encryptedData], payload)
    }

    func testRoundTripMultipleEntries() throws {
        var tlv = TLV8()
    ...
    func testRoundTripLargeFragmentedValue() throws {
    ...
    func testSubscriptReturnsNilForMissingTag() throws {
        let tlv = try TLV8.decode(Data([0x06, 0x01, 0x01]))
        XCTAssertNil(tlv[.publicKey])
    }

    func testSubscriptReturnsReassembledValueForConsecutiveSameTagEntries() throws {
