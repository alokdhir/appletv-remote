import Foundation
import CryptoKit
import BigInt

/// SRP-6a client implementation for HAP (HomeKit Accessory Protocol) pairing.
///
/// Group parameters from RFC 5054 Appendix A — 3072-bit group:
///   N = 3072-bit safe prime
///   g = 5
/// Hash function: SHA-512
/// Username I: "Pair-Setup"
/// Password  P: 4-digit PIN shown on the Apple TV
///
/// Reference: HAP specification §5.6, pyatv protocols/mrp/pairing.py
public struct SRPClient {

    // MARK: - Group parameters (RFC 5054, §A.2 — 3072-bit group)

    public static let N = BigUInt(Data([
        
    ]))

    public static let g = BigUInt(5)
    public static let Nbytes = 384  // byte length of N (3072 bits)
    public static let username = "Pair-Setup"

    // MARK: - Instance state

    /// Client private key (random 256-bit value)
    public let privateKey: BigUInt
    /// Client public key A = g^a mod N (sent to Apple TV)
    public let publicKey: BigUInt

    public init() {
        var a: BigUInt
        repeat {
            a = BigUInt.randomInteger(withExactWidth: 256)
        } while a == 0
        privateKey = a
        publicKey = SRPClient.g.power(a, modulus: SRPClient.N)
    }

    // MARK: - Session key computation

    public struct SessionResult {
        public let sessionKeyK: Data        // K = SHA-512(PAD(S))
        public let clientProof: Data        // M1, sent to Apple TV
        public let expectedServerProof: Data // M2, used to verify Apple TV's response

        public init(sessionKeyK: Data, clientProof: Data, expectedServerProof: Data) {
            self.sessionKeyK = sessionKeyK
            self.clientProof = clientProof
            self.expectedServerProof = expectedServerProof
        }
    }

    /// Compute the SRP session material after receiving the server's salt and public key.
    ///
    /// - Parameters:
    ///   - salt: 16-byte salt from Apple TV's M2 response
    ///   - serverPublicKey: B from Apple TV's M2 response
    ///   - pin: the 4-digit PIN displayed on the Apple TV screen
    public func computeSession(salt: Data, serverPublicKey: Data, pin: String) throws -> SessionResult {
        let N = SRPClient.N
        let g = SRPClient.g
        let len = SRPClient.Nbytes

        let B = BigUInt(serverPublicKey)
        guard B % N != 0 else { throw SRPError.invalidServerPublicKey }

        // k = H(PAD(N) | PAD(g, len(N)))
        let k = BigUInt(sha512(pad(N.toData(), to: len) + pad(g.toData(), to: len)))

        // x = H(salt | H(username ":" pin))
        let credHash = sha512(Data((SRPClient.username + ":" + pin).utf8))
        let x = BigUInt(sha512(salt + credHash))

        // u = H(PAD(A) | PAD(B))
        let u = BigUInt(sha512(pad(publicKey.toData(), to: len) + pad(serverPublicKey, to: len)))
        guard u != 0 else { throw SRPError.invalidScrambling }

        // S = (B - k * g^x mod N) ^ (a + u*x) mod N
        let gx  = g.power(x, modulus: N)
        let kgx = (k * gx) % N
        // Modular subtraction: (B - kgx + N) % N to avoid underflow
        let base = (B + N - kgx) % N
        let exp  = privateKey + u * x
        let S    = base.power(exp, modulus: N)

        // K = SHA-512(S) — srptools/pyatv convention: no padding of S
        let K = sha512(S.toData())

        // M1 = H(H(N) XOR H(g) | H(I) | s | A | B | K)
        // srptools hashes raw g bytes (not padded to group size).
        // H(N) is fine as-is since N.toData() is already 384 bytes.
        let hN   = sha512(N.toData())
        let hg   = sha512(g.toData())   // g = [0x05], 1 byte — NOT padded to 384
        let hNxg = Data(zip(hN, hg).map { $0 ^ $1 })
        let hI   = sha512(Data(SRPClient.username.utf8))
        let M1   = sha512(hNxg + hI + salt
                          + pad(publicKey.toData(), to: len)
                          + pad(serverPublicKey, to: len)
                          + K)

        // M2 (expected server proof) = H(PAD(A) | M1 | K)
        let M2 = sha512(pad(publicKey.toData(), to: len) + M1 + K)

        return SessionResult(sessionKeyK: K, clientProof: M1, expectedServerProof: M2)
    }

    // MARK: - Helpers

    private func sha512(_ data: Data) -> Data {
        Data(SHA512.hash(data: data))
    }

    private func pad(_ data: Data, to length: Int) -> Data {
        if data.count >= length { return data }
        return Data(repeating: 0, count: length - data.count) + data
    }
}

// MARK: - BigUInt ↔ Data

extension BigUInt {
    /// Encode as big-endian bytes (no leading zeros unless empty).
    public func toData() -> Data {
        Data(self.serialize())
    }
}

// MARK: - Error

public enum SRPError: Error {
    case invalidServerPublicKey
    case invalidScrambling
    case serverProofMismatch
}
