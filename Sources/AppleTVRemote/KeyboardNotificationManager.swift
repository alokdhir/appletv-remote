...
final class KeyboardNotificationManager: NSObject {
    ...
    func notify(deviceName: String) {
        if Log.verbose { Log.app.report("notify() called — notified=\(notified) isActive=\(NSApp.isActive)") }
        ...
        if attentionRequestToken == -1 {
            attentionRequestToken = NSApp.requestUserAttention(.informationalRequest)
            if Log.verbose { Log.app.report("notify() bounced — token=\(attentionRequestToken)") }
        }
    }
    ...
    func resetNotify() {
        ...
        NSApp.cancelUserAttentionRequest(attentionRequestToken)
        attentionRequestToken = -1
        notified = false
    }
